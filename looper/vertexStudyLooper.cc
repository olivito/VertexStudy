...

{
  ...
  while((currentFile = (TChainElement*)fileIter.Next())) {
	...
	if (weightvtx > -9000) vtxs_sumpt_recalc_weight.at(weightvtx) += trks_trk_p4().at(itrk).pt();
	...
	if (trk_mcdr().at(itrk) > 0.025) continue;

	// check for tracks matched to the same gen particle..
	if (mc_idxs.count(trk_mcidx().at(itrk)) > 0) {
	  ++mc_idx_duplicates;
	  mc_idxs[trk_mcidx().at(itrk)] += 1;
	}
	else mc_idxs[trk_mcidx().at(itrk)] = 1;
	...
	sum_hardscatter_pt += trks_trk_p4().at(itrk).pt();
      ...
      // loop again to plot dr for duplicate gen matches
      for (unsigned int itrk = 0; itrk < trks_trk_p4().size(); ++itrk) {
	if (trk_mc_id().at(itrk) == -9999) continue;
	...
	if (isGoodVertex(ivtx)) ++nvtx;
      ...
      // plots for vertex 0
      const int vtx0 = 0;
      float frac_hardscatter_dz = vtxs_sumpt_hardscatter_dz.at(vtx0)/vtxs_sumpt_recalc_dz.at(vtx0);
      float frac_hardscatter_weight = vtxs_sumpt_hardscatter_weight.at(vtx0)/vtxs_sumpt_recalc_weight.at(vtx0);
      h_vtx_fracpt_hardscatter_dz->Fill(frac_hardscatter_dz);
      h_vtx_fracpt_hardscatter_weight->Fill(frac_hardscatter_weight);
      h_vtx_fracpt_hardscatter_vs_nvtx->Fill(nvtx,frac_hardscatter_weight);
      h_vtx_hardscatter_fracpt_dz->Fill(vtxs_sumpt_hardscatter_dz.at(vtx0)/sum_hardscatter_pt);
      h_vtx_hardscatter_fracpt_weight->Fill(vtxs_sumpt_hardscatter_weight.at(vtx0)/sum_hardscatter_pt);
      h_vtx_hardscatter_fracpt_vs_nvtx->Fill(nvtx,vtxs_sumpt_hardscatter_weight.at(vtx0)/sum_hardscatter_pt);
      h_vtx_hardscatter_pt_vs_sumpt->Fill(vtxs_sumpt().at(vtx0),vtxs_sumpt_hardscatter_weight.at(vtx0));
      h_vtx_hardscatter_pt_vs_sumpt_recalc->Fill(vtxs_sumpt_recalc_weight.at(vtx0),vtxs_sumpt_hardscatter_weight.at(vtx0));
      h_vtx_sumpt_vs_sumpt_recalc->Fill(vtxs_sumpt_recalc_weight.at(vtx0),vtxs_sumpt().at(vtx0));
...
void vertexStudyLooper::BookHistos(const TString& prefix)
{
  ...
  h_vtx_hardscatter_pt_vs_sumpt = new TH2F(Form("%s_vtx_hardscatter_pt_vs_sumpt",prefix.Data()),";vtx0 #Sigma p_{T};vtx0 #Sigma p_{T} from hard scatter",100,0,1000.,100,0,1000.);
  h_vtx_hardscatter_pt_vs_sumpt_recalc = new TH2F(Form("%s_vtx_hardscatter_pt_vs_sumpt_recalc",prefix.Data()),";vtx0 #Sigma p_{T};vtx0 #Sigma p_{T} from hard scatter",100,0,1000.,100,0,1000.);
  h_vtx_sumpt_vs_sumpt_recalc = new TH2F(Form("%s_vtx_sumpt_vs_sumpt_recalc",prefix.Data()),";vtx0 #Sigma p_{T}, recalc;vtx0 #Sigma p_{T}",100,0,1000.,100,0,1000.);
  h_vtx_fracpt_hardscatter_dz = new TH1F(Form("%s_vtx_fracpt_hardscatter_dz",prefix.Data()),";Frac of vtx0 p_{T} from hard scatter",100,0.,1.);
  h_vtx_fracpt_hardscatter_weight = new TH1F(Form("%s_vtx_fracpt_hardscatter_weight",prefix.Data()),";Frac of vtx0 p_{T} from hard scatter",100,0.,1.);
  h_vtx_fracpt_hardscatter_vs_nvtx = new TH2F(Form("%s_vtx_fracpt_hardscatter_vs_nvtx",prefix.Data()),";N(vtx);Frac of vtx0 p_{T} from hard scatter",50,0,50,100,0.,1.);
  // fraction of total hard scatter pt associated to PV0
  h_vtx_hardscatter_fracpt_dz = new TH1F(Form("%s_vtx_hardscatter_fracpt_dz",prefix.Data()),";Frac of track hard scatter p_{T} assoc to vtx0",100,0.,1.);
  h_vtx_hardscatter_fracpt_weight = new TH1F(Form("%s_vtx_hardscatter_fracpt_weight",prefix.Data()),";Frac of track hard scatter p_{T} assoc to vtx0",100,0.,1.);
  h_vtx_hardscatter_fracpt_vs_nvtx = new TH2F(Form("%s_vtx_hardscatter_fracpt_vs_nvtx",prefix.Data()),";N(vtx);Frac of track hard scatter p_{T} assoc to vtx0",50,0,50,100,0.,1.);
  h_dz_trk_vtx = new TH1F(Form("%s_dz_trk_vtx",prefix.Data()),";dz(trk, best vtx)",1000,-10.,10.);
  ...
  h_mc_idx_duplicates_dr = new TH1F(Form("%s_mc_idx_duplicates_dr",prefix.Data()),";dR(reco,gen) for tracks with duplicate matches",50,0.,0.2);
